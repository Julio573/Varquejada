            ...
            try:
                ...
                with self.controller.horse_tracker.lock:
                    if self.controller.horse_tracker.tracked_horses:
                        # Pega o cavalo com melhor confiança
                        ...
                        data = self.controller.horse_tracker.tracked_horses[best_tid]
                        
                        self.speed_ready.emit(data.get("speed", 0.0))
                        self.distance_ready.emit(data.get("dist_total_m", 0.0))
                    else:
...
class AppController:
    ...
    def on_auto_calibration_success(self, ppm):
        self.set_ppm(ppm)
        self.window.log_list.addItem(f"Calibração Automática OK: {ppm:.2f} px/m")
