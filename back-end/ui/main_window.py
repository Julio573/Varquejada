...
from PyQt6.QtGui import QImage, QPixmap, QFont, QColor, QPainter, QPen, QBrush, QCursor, QShortcut, QKeySequence, QIcon, QPainterPath
...
class SpeedGraph(QWidget):
    """Gráfico de telemetria em tempo real para múltiplos cavalos."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedHeight(120)
        self.data = {} # {id: [velocidades]}
        self.max_points = 100
        self.colors = [QColor("#3B82F6"), QColor("#EF4444"), QColor("#F59E0B"), QColor("#10B981")]
    ...
    def add_data(self, horse_speeds):
        """Recebe dict {id: speed}."""
        for tid, speed in horse_speeds.items():
            if tid not in self.data:
                self.data[tid] = []
            self.data[tid].append(speed)
            if len(self.data[tid]) > self.max_points:
                self.data[tid].pop(0)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        rect = self.rect()
        # Fundo do gráfico
        painter.setBrush(QColor("#0b0d14"))
        painter.setPen(QPen(QColor("#1a1c26"), 1))
        painter.drawRoundedRect(rect, 8, 8)
        
        if not self.data:
            painter.setPen(QColor("#374151"))
            painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, "AGUARDANDO TELEMETRIA...")
            return

        # Desenha Grid
        painter.setPen(QPen(QColor("#1a1c26"), 1, Qt.PenStyle.DashLine))
        for i in range(1, 4):
            y = rect.height() - (i * rect.height() // 4)
            painter.drawLine(0, y, rect.width(), y)

        # Desenha as linhas
        for i, (tid, points) in enumerate(self.data.items()):
            ...
            path = QPainterPath()
            w_step = rect.width() / self.max_points
            
            for j, val in enumerate(points):
                # Normaliza velocidade (assume max 80km/h)
                norm_val = min(val / 80.0, 1.0)
                px = j * w_step
                py = rect.height() - (norm_val * (rect.height() - 10)) - 5
            ...
            painter.drawText(int(rect.width() - 80), 20 + (i * 15), f"CAVALO {tid}")
...
class MainWindow(QMainWindow):
    ...
    def setup_header(self):
        ...
        from ui.header.header_widget import HeaderWidget
        self.header_widget = HeaderWidget()
        self.header_widget.setObjectName("header_frame")
        self.header_widget.close_requested.connect(self.close)
        
        # Redireciona os atributos de label antigos para os novos cards para manter o controller funcionando
        self.lbl_header_elapsed = self.header_widget.card_time.value_label
        self.lbl_header_dist = self.header_widget.card_dist.value_label
        self.lbl_header_date = self.header_widget.card_date.value_label
        self.lbl_header_clock = self.header_widget.card_hour.value_label
        
        self.root_layout.addWidget(self.header_widget)
        
        # Inicia a atualização do relógio local no header
    ...
    def setup_video_player(self):
        # Container externo para o Vídeo (Stack + HUD)
        self.video_container = QWidget()
        self.video_container.setObjectName("video_container")
        self.video_container.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        self.video_container.setMinimumSize(800, 450) # Proporção 16:9 inicial
        ...
        # --- MONTAGEM DO LAYOUT CENTRAL ---
        # Adiciona o container de vídeo com stretch total
        self.video_area_layout.addWidget(self.video_container, stretch=10)
        
        # Gráfico de Telemetria (Abaixo do vídeo, tamanho fixo ou semi-fixo para evitar overlap)
        self.speed_graph = SpeedGraph()
        self.speed_graph.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self.speed_graph.setMinimumHeight(130)
        self.video_area_layout.addWidget(self.speed_graph, stretch=0)
    ...
    def reposition_controls(self):
        """Reposiciona a barra de transporte flutuante de forma responsiva."""
        if not hasattr(self, 'transport_container') or not self.video_container.isVisible():
            return
            
        # Geometria do container de vídeo
        w = self.video_container.width()
        h = self.video_container.height()
        
        # Margens e dimensões da barra (YouTube Style)
        margin = 20
        bar_h = 100
        bar_w = int(w * 0.96) # Ocupa 96% da largura
        
        # Centralização horizontal e fixação no rodapé interno do vídeo
        new_x = (w - bar_w) // 2
        new_y = h - bar_h - margin
        
        self.transport_container.setFixedSize(bar_w, bar_h)
        self.transport_container.move(new_x, new_y)
        self.transport_container.raise_()

    def resizeEvent(self, event):
        """Gerencia o redimensionamento da janela e reposiciona elementos flutuantes."""
    ...
    def toggle_fullscreen(self):
        if self.isFullScreen():
            ...
            # Garante que os controles apareçam/sumam corretamente em full
            self.show_controls()
        
        QTimer.singleShot(100, self.reposition_controls)
